<?php
/**
 * Chic Lite Typography Related Functions
 *
 * @package Chic_Lite
 */

if( ! function_exists( 'chic_lite_fonts_url' ) ):
/**
 * 
*/ 
function chic_lite_fonts_url(){
    ...
    */
    $primary    = _x( 'on', 'Primary Font: on or off', 'chic-lite' );
    $secondary  = _x( 'on', 'Secondary Font: on or off', 'chic-lite' );
    $site_title = _x( 'on', 'Site Title Font: on or off', 'chic-lite' );
...
if( ! function_exists( 'chic_lite_get_google_fonts' ) ) :
/**
 * Get Google Fonts
*/
function chic_lite_get_google_fonts(){
...
if( ! function_exists( 'chic_lite_get_websafe_font' ) ) :
/**
 * Function listing WebSafe Fonts and its attributes
*/
function chic_lite_get_websafe_font(){
    ...
    return apply_filters( 'chic_lite_standard_fonts', $standard_fonts );
}
endif;

if( ! function_exists( 'chic_lite_check_varient' ) ) :
/**
 * Checks for matched varients in google fonts for typography fields
*/
function chic_lite_check_varient( $font_family = 'serif', $font_variants = 'regular', $body = false ){
    $variant = '';
    $var     = array();
    $google_fonts  = chic_lite_get_google_fonts(); //Google Fonts
    $websafe_fonts = chic_lite_get_websafe_font(); //Standard Web Safe Fonts
...
*/
function chic_lite_get_css_variant( $font_variant ){
    $v_array = array(
		'100'       => array(
            'weight'    => '100',
            'style'     => 'normal'
...
*/
function chic_lite_is_google_font( $font ){
    $return = false;
    $websafe_fonts = chic_lite_get_websafe_font();
    if( $font ){
...
*/
function chic_lite_get_fonts( $font_family, $font_variant ){
    
    $fonts = array();
    $websafe_fonts = chic_lite_get_websafe_font(); //Web Safe Font
    
    if( $font_family ){
        if( chic_lite_is_google_font( $font_family ) ){
            $fonts['font'] = esc_attr( $font_family ); //Google Font
            if( $font_variant ){
                $weight_style    = chic_lite_get_css_variant( chic_lite_check_varient( $font_family, $font_variant ) );
...
if( ! function_exists( 'chic_lite_get_all_fonts' ) ) :
/**
 * Return Web safe font and google font without variants
*/
function chic_lite_get_all_fonts(){
    $google = array();        
    $standard = apply_filters( 'chic_lite_standard_font', array(
		'georgia-serif'       => __( 'Georgia', 'chic-lite' ),
        'palatino-serif'      => __( 'Palatino Linotype, Book Antiqua, Palatino', 'chic-lite' ),
        'times-serif'         => __( 'Times New Roman, Times', 'chic-lite' ),
        'arial-helvetica'     => __( 'Arial, Helvetica', 'chic-lite' ),
        'arial-gadget'        => __( 'Arial Black, Gadget', 'chic-lite' ),
		'comic-cursive'       => __( 'Comic Sans MS, cursive', 'chic-lite' ),
		'impact-charcoal'     => __( 'Impact, Charcoal', 'chic-lite' ),
        'lucida'              => __( 'Lucida Sans Unicode, Lucida Grande', 'chic-lite' ),
        'tahoma-geneva'       => __( 'Tahoma, Geneva', 'chic-lite' ),
		'trebuchet-helvetica' => __( 'Trebuchet MS, Helvetica', 'chic-lite' ),
		'verdana-geneva'      => __( 'Verdana, Geneva', 'chic-lite' ),
        'courier'             => __( 'Courier New, Courier', 'chic-lite' ),
        'lucida-monaco'       => __( 'Lucida Console, Monaco', 'chic-lite' ),
	) );
